...
async function importExcel() {
    console.log('--- MANUAL IMPORT V3 START ---');
    ...
    try {
        ...
        const findSatkerId = (excelName) => {
            if (!excelName) return officialSatkers[0].id;
            ...
            if (search.includes('brimob')) return satkerNames.find(s => s.name.includes('brimob'))?.id || officialSatkers[0].id;
        ...
        const extractDate = (text) => {
            if (!text) return null;
            // Seek "TGL 17 SEP 2015" or similar
        ...
        for (const row of dataRows) {
            ...
            const satkerId = findSatkerId(satkerExcel);
        ...
        console.log('--- IMPORT V3 FINISHED ---');
